...
int os_proc_children(int ppid, int **proc_list, size_t *proc_count) FUNC_ATTR_NONNULL_ALL
{
  if (ppid < 0) {
    return 2;
  }
...
bool os_proc_tmux_info(int pid, char msg[static 512])
{
  ...
  char proc_env[256] = { 0 };
  if (snprintf(proc_env, sizeof(proc_env), "/proc/%d/environ", pid) >= (long)sizeof(proc_env)) {
    return false;
  }
  ...
  if (statbuf.st_size != 0 || (statbuf.st_mode & S_IFMT) != S_IFREG) {
  ...
  while ((tok = memchr(p, '\0', (size_t)(p_end - p)))) {
    ...
    if (eq != NULL) {
      ...
      if (strncmp(p, "TMUX_PANE=", tok_len) == 0) {
        if (sscanf(eq, "%255s", pane_id) != 1 || strlen(pane_id) == 0) {
          goto error;
        }
      }
      ...
      if (strncmp(p, "TMUX=", tok_len) == 0) {
        ...
        if (sscanf(eq, "%255[^,],%d,", socket_path, &server_pid) != 2 || strlen(socket_path) == 0
            || server_pid == 0 || !os_proc_running(server_pid)) {
          goto error;
        }
      }
    }
...
# undef ADD_ARG
  ...
  if (os_system(argv, NULL, 0, &tmux_call_output, &nread) < 0 || tmux_call_output == NULL
      || nread == 0) {
